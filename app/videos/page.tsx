...
import { VIDEOS } from '@/app/data/videos'; // ЭТОТ МОСТ БЕРЕТ ДАННЫЕ
      ...
      <div className="w-full flex flex-col md:flex-row flex-grow mt-12 md:mt-16 gap-8 items-start mb-auto px-4">
        ...
        <nav className="w-full md:w-48 flex flex-row md:flex-col flex-wrap gap-x-4 gap-y-1 text-[11px] uppercase tracking-wider text-left border-b md:border-b-0 pb-4 md:pb-0 border-gray-200">
          {menuFilters[currentLang].map((filter) => (
            <button key={filter.id} onClick={() => setActiveFilter(filter.id)} className={`block py-0.5 font-bold ${activeFilter === filter.id ? 'text-black underline' : 'text-gray-400 hover:text-black'}`}>
              {filter.label}
            </button>
          ))}
        </nav>
            ...
            {VIDEOS.filter(v => activeFilter === 'all' || v.category === activeFilter).map((video) => (
              video.isComingSoon ? (
                ...
                <Link href={`/videos/${video.id}`} key={video.id} className="flex flex-col space-y-2 group cursor-pointer">
                  <div className="aspect-square bg-white border border-gray-100 flex items-center justify-center overflow-hidden relative group-hover:border-gray-400 transition-all">
                    <img src={video.cover} alt="" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-300" />
                    <div className="absolute bg-white/90 text-black text-[10px] tracking-widest font-bold px-2 py-1 uppercase opacity-0 group-hover:opacity-100 transition-opacity">PLAY</div>
                  </div>
                  <div className="flex flex-col text-[10px] uppercase tracking-wider space-y-0.5">
